# Stock Viewer 使用指南

本文档详细说明项目中各个 npm 命令的用途、流程和最佳实践。

## 📋 目录

- [基础编译命令](#基础编译命令)
- [打包命令 (package)](#打包命令-package)
- [版本管理命令 (version)](#版本管理命令-version)
- [发布命令 (publish)](#发布命令-publish)
- [生命周期钩子](#生命周期钩子)
- [package、version、publish 的关联和区别](#packageversionpublish-的关联和区别)
- [实际使用流程](#实际使用流程)
- [最佳实践建议](#最佳实践建议)

---

## 🔧 基础编译命令

### 1. `compile`

```bash
npm run compile
```

- **用途**: 编译 TypeScript 代码为 JavaScript
- **原理**: 执行 `tsc -p ./`，根据 `tsconfig.json` 配置编译
- **输出**: 生成 `out/` 目录中的 `.js` 文件
- **场景**: 
  - 发布前确保代码已编译
  - 手动触发编译

### 2. `watch`

```bash
npm run watch
```

- **用途**: 监听模式，自动编译修改的文件
- **场景**: 开发时使用，保存代码后自动编译
- **原理**: `tsc -watch -p ./` 持续监听文件变化
- **提示**: 在终端保持运行，按 `Ctrl+C` 停止

### 3. `build`

```bash
npm run build
```

- **用途**: 构建项目（等同于 compile）
- **原理**: 调用 `npm run compile`
- **场景**: 发布前的标准构建步骤

### 4. `clean`

```bash
npm run clean
```

- **用途**: 清理编译产物
- **原理**: `rimraf out *.vsix` 删除 `out/` 目录和所有 `.vsix` 文件
- **场景**: 
  - 重新打包前清理旧文件
  - 解决编译缓存问题
  - 准备全新构建

---

## 📦 打包命令 (package)

### 5. `package`

```bash
npm run package
```

- **用途**: 打包扩展为 `.vsix` 文件（用于本地安装或手动分发）
- **流程**: 
  1. `npm run build` - 编译代码
  2. `vsce package` - 打包成 `.vsix` 文件
- **输出**: 生成 `stock-viewer-x.x.x.vsix` 文件
- **场景**: 
  - 本地测试安装（通过 VS Code 的"从 VSIX 安装"）
  - 离线分发给其他人
  - 内网或私有部署

**如何安装 .vsix 文件：**
1. 打开 VS Code
2. 按 `Ctrl+Shift+P` (Mac: `Cmd+Shift+P`)
3. 输入 "Install from VSIX"
4. 选择生成的 `.vsix` 文件

### 6. `package:patch`

```bash
npm run package:patch
```

- **用途**: 升级 **patch** 版本号并打包
- **流程**:
  1. `npm version patch --no-git-tag-version` - 版本号 +0.0.1
  2. `npm run package` - 打包
- **示例**: `0.0.1` → `0.0.2`
- **适用**: Bug 修复、小改进

### 7. `package:minor`

```bash
npm run package:minor
```

- **用途**: 升级 **minor** 版本号并打包
- **示例**: `0.0.1` → `0.1.0`
- **适用**: 新功能添加、向后兼容的更改

### 8. `package:major`

```bash
npm run package:major
```

- **用途**: 升级 **major** 版本号并打包
- **示例**: `0.1.0` → `1.0.0`
- **适用**: 破坏性更改、重大重构

---

## 🔢 版本管理命令 (version)

### 9. `version:patch`

```bash
npm run version:patch
```

- **用途**: 仅升级 patch 版本号（不打包、不发布）
- **原理**: `npm version patch --no-git-tag-version`
- **修改**: 直接修改 `package.json` 中的 `version` 字段
- **场景**: 想手动控制打包和发布时机

### 10. `version:minor`

```bash
npm run version:minor
```

- **用途**: 仅升级 minor 版本号
- **示例**: `0.0.1` → `0.1.0`

### 11. `version:major`

```bash
npm run version:major
```

- **用途**: 仅升级 major 版本号
- **示例**: `0.1.0` → `1.0.0`

---

## 🚀 发布命令 (publish)

### 12. `publish`

```bash
npm run publish
```

- **用途**: 发布扩展到 VS Code Marketplace（不升级版本）
- **流程**:
  1. `npm run build` - 编译代码
  2. `vsce publish` - 发布到市场
- **前提**: 
  - ✅ 版本号必须比已发布的版本高
  - ✅ 需要有效的 Personal Access Token (PAT)
  - ✅ Git 工作区应该是干净的
- **场景**: 已手动升级过版本号

### 13. `publish:patch`

```bash
npm run publish:patch
```

- **用途**: 升级 patch 版本并发布到 Marketplace
- **流程**:
  1. `npm run version:patch` - 版本号 +0.0.1
  2. `npm run build` - 编译
  3. `vsce publish patch` - 发布
- **示例**: `0.0.1` → `0.0.2` 并发布
- **适用**: Bug 修复、文档更新

### 14. `publish:minor`

```bash
npm run publish:minor
```

- **用途**: 升级 minor 版本并发布
- **示例**: `0.0.1` → `0.1.0` 并发布
- **适用**: 新功能添加

### 15. `publish:major`

```bash
npm run publish:major
```

- **用途**: 升级 major 版本并发布
- **示例**: `0.1.0` → `1.0.0` 并发布
- **适用**: 重大更新、破坏性更改

---

## 🔄 生命周期钩子

### 16. `vscode:prepublish`

```bash
# 自动执行，不需要手动调用
```

- **用途**: 发布前自动执行的钩子
- **时机**: `vsce publish` 或 `vsce package` 之前自动触发
- **原理**: 调用 `npm run compile` 确保代码已编译
- **重要**: 这是 VS Code 扩展的标准钩子

### 17. `prepublishOnly`

```bash
# 自动执行，不需要手动调用
```

- **用途**: npm publish 前执行（VS Code 扩展一般不用）
- **原理**: 清理旧文件并重新构建
- **注意**: 与 `vscode:prepublish` 不同

---

## 🔄 package、version、publish 的关联和区别

### 📊 对比表格

| 命令类型 | 版本升级 | 编译代码 | 打包 .vsix | 发布到市场 | 使用场景 |
|---------|---------|---------|-----------|-----------|---------|
| **version** | ✅ | ❌ | ❌ | ❌ | 只想改版本号 |
| **package** | ❌ | ✅ | ✅ | ❌ | 本地测试安装 |
| **publish** | ❌ | ✅ | ❌ | ✅ | 发布到市场 |
| **package:patch** | ✅ | ✅ | ✅ | ❌ | 升级版本+打包 |
| **publish:patch** | ✅ | ✅ | ❌ | ✅ | 升级版本+发布 |

### 🔍 详细区别

#### 1️⃣ **version 命令**

```bash
npm run version:patch   # 0.0.1 → 0.0.2
```

- **只做一件事**: 修改 `package.json` 中的版本号
- **不编译、不打包、不发布**
- **适用场景**: 
  - 想先改版本号，稍后再打包/发布
  - 批量处理多个操作前先统一版本号

#### 2️⃣ **package 命令**

```bash
npm run package         # 生成 stock-viewer-0.0.1.vsix
npm run package:patch   # 升级版本 → 0.0.2 → 生成 .vsix
```

- **目标**: 生成 `.vsix` 文件
- **用途**: 
  - ✅ 本地安装测试（通过 VS Code 的"从 VSIX 安装"）
  - ✅ 离线分发给其他人
  - ✅ 内网或私有部署
- **不发布到市场**: 文件只在本地
- **对比 publish**: 不需要市场账号和 token

#### 3️⃣ **publish 命令**

```bash
npm run publish         # 发布当前版本到市场
npm run publish:patch   # 升级版本 → 0.0.2 → 发布
```

- **目标**: 发布到 VS Code Marketplace
- **要求**:
  - ✅ 需要 marketplace 账号
  - ✅ 需要 Personal Access Token (PAT)
  - ✅ 版本号必须比市场上的版本高
- **用途**: 
  - ✅ 公开发布，所有用户可搜索安装
  - ✅ 自动更新提醒
- **不生成 .vsix 文件**: 直接发布到云端

---

## 🎯 实际使用流程

### 场景1: 日常开发测试

```bash
# 1. 启动监听模式
npm run watch

# 2. 修改代码...
# 代码会自动编译

# 3. 打包测试
npm run package

# 4. 在 VS Code 中安装 .vsix 测试
# Extensions → Install from VSIX → 选择生成的文件

# 5. 测试功能
```

### 场景2: 发布小更新（Bug修复）

```bash
# 1. 确保所有修改已提交
git add .
git commit -m "fix: 修复某个bug"

# 2. 升级 patch 版本并发布
npm run publish:patch
# 0.0.1 → 0.0.2 并自动发布

# 3. 推送代码到 GitHub
git push
```

### 场景3: 发布新功能

```bash
# 1. 提交新功能代码
git add .
git commit -m "feat: 新增某功能"

# 2. 升级 minor 版本并发布
npm run publish:minor
# 0.0.1 → 0.1.0 并自动发布

# 3. 推送代码
git push
```

### 场景4: 先打包测试，确认无误再发布

```bash
# 1. 升级版本并打包
npm run package:patch
# 生成 stock-viewer-0.0.2.vsix

# 2. 本地测试 .vsix 文件
# 在 VS Code 中安装并测试所有功能

# 3. 确认无误后发布到市场
npm run publish
# 使用已升级的版本号发布

# 4. 提交代码
git add .
git commit -m "chore: 发布 v0.0.2"
git push
```

### 场景5: 分步操作（精细控制）

```bash
# 1. 先改版本号
npm run version:minor
# 0.0.1 → 0.1.0

# 2. 编译代码
npm run build

# 3. 打包测试
npm run package

# 4. 本地测试...

# 5. 确认后发布
npm run publish

# 6. 提交代码
git add .
git commit -m "chore: 发布 v0.1.0"
git push
```

### 场景6: 清理重建

```bash
# 1. 清理所有编译产物
npm run clean

# 2. 重新编译
npm run build

# 3. 打包
npm run package
```

---

## 💡 最佳实践建议

### ✅ 推荐做法

#### 1. **选择合适的版本类型**
- **Patch (0.0.x)**: Bug 修复、文档更新、性能优化
- **Minor (0.x.0)**: 新功能、向后兼容的改进
- **Major (x.0.0)**: 破坏性更改、重大架构调整

#### 2. **发布流程**
```bash
# 标准流程
git add .
git commit -m "feat: 新功能描述"
npm run publish:minor  # 或 patch/major
git push
```

#### 3. **发布前测试**
```bash
# 先打包测试
npm run package:patch
# 测试 .vsix
# 确认无误后
npm run publish
```

#### 4. **保持 Git 干净**
- 发布前确保所有修改已提交
- 避免在未提交状态下发布
- 使用有意义的 commit 信息

#### 5. **版本号规范**
遵循语义化版本（Semantic Versioning）：
- **Major.Minor.Patch** (例如：1.2.3)
- 不要随意跳跃版本号
- 保持版本历史连贯

### ❌ 避免的做法

#### 1. **不要重复发布相同版本**
```bash
# ❌ 错误示例
npm run publish  # 版本 0.0.1
npm run publish  # 还是 0.0.1，会报错
```
**解决**: 必须先升级版本号

#### 2. **不要在 Git 工作区不干净时升级版本**
```bash
# ❌ 错误示例
# 有未提交的文件
npm run publish:patch  # 可能导致版本不一致
```
**解决**: 先提交所有修改

#### 3. **不要跳过测试直接发布**
```bash
# ❌ 错误示例
# 修改代码后直接
npm run publish:minor  # 未测试就发布
```
**解决**: 先用 `npm run package` 打包测试

#### 4. **不要在开发分支发布**
```bash
# ❌ 错误示例
git checkout dev
npm run publish:patch  # 在开发分支发布
```
**解决**: 切换到 main/master 分支发布

---

## 🔍 常见问题解答

### Q1: 发布失败：版本已存在

**错误信息**: `ERROR MuWeiLiu.stock-viewer v0.0.4 already exists.`

**解决方法**:
```bash
# 升级到新版本
npm run publish:patch  # 或 minor/major
```

### Q2: Git 工作区不干净

**错误信息**: `Git working directory not clean`

**解决方法**:
```bash
# 提交所有修改
git add .
git commit -m "描述信息"

# 或者暂存修改
git stash
npm run publish:patch
git stash pop
```

### Q3: Node.js 版本不匹配

**错误信息**: `ReferenceError: File is not defined`

**原因**: Node.js 版本太低，项目要求 Node.js 22

**解决方法**:
```bash
# 切换到正确的 Node.js 版本
nvm use

# 或者手动安装并切换
nvm install 22
nvm use 22
```

### Q4: vscode 版本不匹配

**错误信息**: `@types/vscode greater than engines.vscode`

**解决方法**:
在 `package.json` 中升级 `engines.vscode`:
```json
"engines": {
  "vscode": "^1.85.0"
}
```

### Q5: 如何撤销已发布的版本？

**不能直接撤销**，但可以：
1. 发布新的修复版本
2. 通过 Marketplace 管理后台标记为弃用
3. 联系 VS Code 团队删除（特殊情况）

### Q6: .vsix 文件在哪里？

**位置**: 项目根目录
**命名**: `stock-viewer-x.x.x.vsix`
**清理**: `npm run clean` 会删除

### Q7: 发布前的检查清单

在运行发布命令前，确保：
- ✅ Node.js 版本正确（使用 `nvm use` 切换到项目要求的版本）
- ✅ Git 工作区干净（所有修改已提交）
- ✅ 代码已编译通过（`npm run build`）
- ✅ 本地测试通过（`npm run package` 并安装测试）
- ✅ 版本号符合语义化规范

---

## 🔑 关键总结

### 命令速查

```bash
# 开发阶段
npm run watch          # 监听编译
npm run compile        # 手动编译
npm run clean          # 清理产物

# 测试阶段
npm run package        # 打包测试
npm run package:patch  # 升级版本+打包

# 发布阶段
npm run publish:patch  # Bug修复发布 (0.0.x)
npm run publish:minor  # 新功能发布 (0.x.0)
npm run publish:major  # 重大更新发布 (x.0.0)
```

### 完整发布流程

```bash
# 1. 开发 → 2. 测试 → 3. 提交 → 4. 发布 → 5. 推送

git add .
git commit -m "feat: 新功能"
npm run publish:minor
git push
```

### 核心理念

- **version** = 只改版本号（准备阶段）
- **package** = 打包本地文件（测试阶段）
- **publish** = 发布到市场（发布阶段）

**快捷方式**: `publish:patch/minor/major` 一条命令搞定所有步骤！🚀

---

## 📚 相关链接

- [VS Code Extension API](https://code.visualstudio.com/api)
- [vsce CLI 文档](https://github.com/microsoft/vscode-vsce)
- [语义化版本规范](https://semver.org/lang/zh-CN/)
- [项目 README](./README.md)

---

**最后更新**: 2025-11-01

